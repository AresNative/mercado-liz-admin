...
export const Column: React.FC<ColumnProps> = ({ status, tasks, activeTask, overColumn, statusColumns, refetch }) => {
    ...
    return (
        <div
            ref={setNodeRef}
            className={`min-w-80 w-80 min-h-[40rem] bg-white dark:bg-zinc-800 p-4 rounded-lg shadow-md transition-all duration-200 ease-in-out hover:shadow-lg ${borderColorClass} border-l-4`}
        >
            ...
            <div className="relative mt-2">
                {showDropArea && (
                    <div className="absolute min-h-[35rem] top-0 left-0 right-0 border-2 border-dashed border-gray-400 rounded-lg p-4 mb-2 bg-zinc-100 z-10">
                        <span className="text-gray-600 dark:text-white font-medium">Soltar aquí</span>
                    </div>
                )}
